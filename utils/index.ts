...
export async function fetchCars(model: string) {
  const headers:HeadersInit = {
    "X-RapidAPI-Key": "605dfd6d94mshf59419cc264a3e3p1ea988jsn795e095270de",
...
export function getCarRent(city_mpg:number,year:number){
  const basePricePerDay = 50; // Base rental price per day in dollars
  const mileageFactor = 0.3; // Additional rate per mile driven
  const ageFactor = 0.15; // Additional rate per year of vehicle age
